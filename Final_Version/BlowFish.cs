...
namespace BlowFish_namespace
{
    ...
    public sealed class BlowFish : IDisposable
    {
...
#if NETCOREAPP
            return Encoding.UTF8.GetString(this.DecryptCBC(HexToByte(ct.Substring(16)))).Replace("\0", string.Empty, StringComparison.Ordinal);
#else
        ...
        public string EncryptECB(string pt)
            => ByteToHex(this.EncryptECB(Encoding.UTF8.GetBytes(pt)));
...
#if NETCOREAPP
            return Encoding.UTF8.GetString(this.Decrypt_ECB(HexToByte(ct))).Replace("\0", string.Empty, StringComparison.Ordinal);
#else
            return Encoding.UTF8.GetString(this.Decrypt_ECB(HexToByte(ct))).Replace("\0", string.Empty);
#endif
        }
